...
typedef union _POINTER
{
    struct
    {
        unsigned char       bLow;
        unsigned char       bHigh;
        unsigned char       bUpper;
    };
    unsigned int            _word;         // bLow & bHigh
    unsigned char*          bRam;          // Ram byte pointer: 2 bytes pointer pointing
                                           // to 1 byte of
    unsigned int*           wRam;          // Ram word poitner: 2 bytes poitner pointing
                                           // to 2 bytes of data
    unsigned char*          bRom;          // Size depends on compiler setting
    unsigned int*           wRom;
} POINTER;
...
typedef union _CTRL_TRF_SETUP
{
    ...
    struct
    {
        unsigned char       _byte[EP0_BUFF_SIZE];
    };
    ...
    struct
    {
        ...
        unsigned char       bDscType;       //Device,Configuration,String
